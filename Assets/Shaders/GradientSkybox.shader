...
Shader "Skybox/GradientSkybox"
{
	Properties
	{
		...
		_Banding("Banding", Range(0.0, 1.0)) = 1.0
	...
	fixed4 frag(v2f i) : COLOR
	{
		...
		float4 color = lerp(_Color1, _Color2, pow(d, _Exponent)) * _Intensity;
		color = float4(color.x % _Banding, color.y % _Banding, color.z % _Banding, color.w);
		return color;
	}
